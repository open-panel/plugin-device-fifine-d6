...
Runtime dependencies (`@openpanel/plugin-sdk`, `@openpanel/device-sdk`,
`@openpanel/shared`, `node-hid`) come **with OpenPanel**. You don't install
them yourself.
...
1. Make sure it shows up as `HOTSPOTEKUSB HID DEMO`, vendor `0x3142`, product
   ...
   ```bash
   pnpm --filter @openpanel/cli dev -- devices scan-hid
   ```
...
Or from the OpenPanel root: `pnpm --filter @openpanel/plugin-fifine-d6 test`.
